...
| Expired sessions/games | Once a session's date + `toTime` passes, it's filtered out of the list read path (`getSessionsForTeam`) — not deleted, just no longer shown, for every status and kind | User reported a past test game still cluttering `/team/[slug]` days later; a hard-delete would need a new scheduled job (Vercel Cron, a new API route) for a group this size that doesn't yet have any cleanup habit to rely on. Hiding costs nothing extra to build, is reversible in principle, and matches how cancelled sessions are already treated (kept, not deleted) — confirmed with the user over a hide-vs-delete fork |
