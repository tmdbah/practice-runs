...
import { PATCH } from "@/app/api/teams/[slug]/sessions/[sessionId]/confirm/route";
...
function makeRequest(): Request {
  return new Request(
    "http://localhost/api/teams/demo-team/sessions/s1/confirm",
    { method: "PATCH" },
  );
}
...
describe("PATCH /api/teams/[slug]/sessions/[sessionId]/confirm", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("should return 400 when the session is already cancelled", async () => {
    ...
    const body = await res.json();
    expect(body.error).toBe("Cannot confirm a cancelled session");
    expect(mockSessionUpdate).not.toHaveBeenCalled();
  });

  it("should no-op and return 200 when already confirmed", async () => {
    mockSessionFindFirst.mockResolvedValueOnce(
      makeSessionRow({ status: "CONFIRMED" }) as never,
    );
  ...
  it("should transition a proposed session to confirmed and return 200", async () => {
    const res = await PATCH(makeRequest(), makeParams());
