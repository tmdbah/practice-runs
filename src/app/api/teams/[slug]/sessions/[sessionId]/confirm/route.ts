...
/**
 * Marks a session as booked for sure. Proposer-only in the UI, not enforced
 * server-side (matches Edit/Delete/RSVP's trust model). Idempotent when
 * already CONFIRMED; rejected when CANCELLED — propose a new session instead.
 */
export async function PATCH(
  ...
  if (existing.status === "CANCELLED") {
    return NextResponse.json(
      { error: "Cannot confirm a cancelled session" },
      { status: 400 },
    );
  }
