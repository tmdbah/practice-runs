...
**Also shipped (ahead of/alongside Phase 4):** Session notifications, layer 1 of 3 — a shareable `/team/[slug]/sessions/[sessionId]` link (identity-free viewing, identity-gated RSVP, Share button with Web Share API + clipboard fallback). Web Push and SMS (the other two brainstormed layers) are deliberately deferred — see the Decisions log in `practiceRuns-ProjectOverview.md`. Also shipped: **Game Day** — a `Session.kind` (`PRACTICE`/`GAME`) discriminator adds league-game coordination alongside practice sessions, surfaced as its own "Game Day" section above "Sessions" on `/team/[slug]`. Games are never rentals (cost UI is hidden regardless of venue), and `minPlayers` means "minimum to avoid forfeiting" (distinct red/urgent wording when short) rather than "worth booking."
